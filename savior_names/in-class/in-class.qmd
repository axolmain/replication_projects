...
```{python}
def dist_names(dat):
    text = list(dat.select(pl.col("scripture_text").str.to_lowercase().str.concat(" "))["scripture_text"])[0].str.split(sep=saviors_names).unlist()
...
```
```{python}
aviors_names = list(names.select(pl.col("name").unique().sort().str.to_lowercase().str.concat('|'))["name"])[0]
...
dist_between_bom = nested_bom.with_columns([
    nested_bom["scripture_text"].map(dist_names)
])
